---
<img width="369" height="122" alt="4cbf8a5c95a08391aba17865ff6081be" src="https://github.com/user-attachments/assets/765e0f19-e207-4091-bdcb-29f29a7da36e" />

---

### sobre mim
Me chamo Jack Baptista, estudante de Ciência de Dados e Neurociências na Universidade Federal do ABC, atualmente construindo a interseção entre neurociência computacional e aprendizado de máquina - particularmente interessado em como LLMs e interfaces cérebro-computador se relacionam com a forma como os sistemas biológicos processam informações.

Eu aprendo melhor praticando antes de ir para a teoria: Python veio da análise de dados, C veio da modificação de um PSP e da configuração do Linux, e o Arch Linux veio do desejo de realmente entender o que está sendo executado na minha máquina. A maioria dos meus projetos parte de uma questão prática e acaba indo mais fundo do que o planejado.

Fora dos dados, fiz workshops de modelagem 3D, participei de um simpósio de neurociência e cognição onde trabalhei com análise de interface cérebro-máquina em Python e estudei coisas como arquivologia e ferramentas de produtividade – principalmente porque acho difícil não ter curiosidade sobre como as informações são estruturadas e usadas.

---

### atualmente

- Projetos EDA que combinam conjuntos de dados comportamentais e institucionais
- Explorando modelagem estatística, probabilidade e reconhecimento de padrões
- Construindo um portfólio como ciêntista de dados, pesquisador acadêmico e entusiasta tech
- Aprendendo como as coisas funcionam uma camada mais profunda do que o necessário

---

### contatos

Aberto a oportunidades de estágio em dados, análise ou pesquisa aplicada;
para mais informações:

- Email profissional: jacksonbaptistall@gmail.com
- Currículo em pdf: [aqui](./currículo.pdf)



---

### setup atual

- notebook samsung flash f30 64gb Intel Celeron N4000
- teclado logitech K235 100% cinza
- mouse sem fio new ion mk06-c03 transparente
- air30 ultrapods max

----

